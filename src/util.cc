...
struct ExternalDataInfo {
  ...
  ExternalDataInfo(const onnx::TensorProto* tensor) {
  ...
  void setExternalData(onnx::TensorProto* tensor, int32_t size_threshold = 0) {
    ...
    if (!tensor->has_raw_data()) {
      if (!onnx::is_processor_little_endian()) {
        return;
      }
#define DO_CASE(pb_type, storage_field, cpp_type)                 \
  case onnx::TensorProto_DataType_##pb_type: {                          \
...
std::vector<onnx::TensorProto*> getInitializerTensors(onnx::ModelProto* m) {
...
std::vector<onnx::TensorProto*> getAttributeTensors(onnx::ModelProto* m) {
  std::vector<onnx::TensorProto*> tensors;
...
std::vector<onnx::TensorProto*> getAllTensors(onnx::ModelProto* m) {
...
bool usesExternalData(onnx::TensorProto* tensor) {
  return tensor->has_data_location() &&
         tensor->data_location() == onnx::TensorProto_DataLocation_EXTERNAL;
}

void loadExternalDataForTensor(onnx::TensorProto* tensor,
...
void loadExternalDataForModel(onnx::ModelProto* m,
                              const std::filesystem::path& base_dir) {
...
void saveExternalData(onnx::TensorProto* tensor,
                      const std::filesystem::path& base_dir) {
  ExternalDataInfo info(tensor);
  auto external_data_file_path = base_dir;
  external_data_file_path /= (info.location);
...
void convertModelToExternalData(onnx::ModelProto* m, const std::string& location = {},
...
void writeExternalDataTensors(onnx::ModelProto* m,
                              const std::filesystem::path& base_dir) {
  auto tensors = getAllTensors(m);
  for (auto& tensor : tensors) {
...
void loadModel(onnx::ModelProto* m, const std::string& model_path,
  ...
  onnx::LoadProtoFromPath<onnx::ModelProto>(model_path, *m);
