...
int main() {
    std::string filename = "yolos_encoder_1_heads.onnx"; // Replace with your ONNX model file path
    ...
    std::vector<std::string> pass_names = {"eliminate_identity"};
    frcc::Optimizer opt(pass_names);
    auto m = opt.optimize(model);

    saveOnnxModel(m, "yolos2_without_identity.onnx");
    return 0;
}